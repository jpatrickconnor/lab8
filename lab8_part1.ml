...
module MakeBestInterval (Endpoint : ORDERED_TYPE)
  ...
  struct
    ...
    type interval =
                    ...
                    | Empty
    (* create low high -- Returns a new interval covering `low` to
       `high` inclusive. If `low` > `high`, then the interval is       empty. *)    let create (low : endpoint) (high : endpoint) : interval =      if Endpoint.compare low high > 0 then Empty      else Interval (low, high)    (* is_empty intvl -- Returns true if and only if the `intvl` is       empty *)    let is_empty (intvl : interval) : bool =      match intvl with      | Empty -> true      | Interval _ -> false    (* contains intvl x -- Returns true if and only if the value `x`       is contained within `intvl` *)
    let contains (intvl : interval) (x : Endpoint.t) : bool =
      match intvl with
      | Empty -> false
      | Interval (low, high) ->
